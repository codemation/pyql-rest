#!/bin/bash
#Usage ./restart_pyql_rest.sh <tag> <local-port> <clusterhost:port> <db-port> <cluster> <init|join|rejoin|test> [join token]  [--no-cache]

#restart mysql db endpoint
echo "1. restarting / recreating mysql container on port "$5
./docker_db/start_db.sh mysql pyql-rest-$2 $4 $6
...
then
    ...
    python test.py $ip $4 joshdb mysql josh abcd1234
fi
...
docker build docker_pyql_rest/ -t joshjamison/pyql-rest:$1 $8

action=$(echo $6 | grep 'join' > /dev/null && echo -n 'join' || echo -n 'init')
env="-e PYQL_TYPE=K8S -e PYQL_VOLUME_PATH=/mnt/pyql-rest -e PYQL_PORT=$2 -e PYQL_CLUSTER_SVC=$3 -e PYQL_HOST=$ip" 
env1="-e PYQL_CLUSTER_NAME=$5 -e PYQL_CLUSTER_ACTION=$action -e PYQL_CLUSTER_TABLES=ALL -e PYQL_DEBUG=Enabled"
env2="-e PYQL_CLUSTER_JOIN_TOKEN=$7"
env3="-e DB_USER=josh -e DB_PASSWORD=abcd1234 -e DB_HOST=$ip -e DB_PORT=$4 -e DB_NAMES=joshdb -e DB_TYPE=mysql"
