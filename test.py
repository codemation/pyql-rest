...
def test(db):
    ...
    assert str(type(db)) == "<class 'pyql.data.database'>", f"failed to create data.database object - found {str(type(db))}"
...
if __name__ == '__main__':
    ...
    maxConnectRetry = 3
    tryCount = 1
    while tryCount <= maxConnectRetry:
        try:
            db = data.database(mysql.connector.connect, **config)
            break
        except Exception as e:
            print(f"exception setting up db object - {repr(e)} - will retry {tryCount} after 10 sec")
        time.sleep(10)
        tryCount+=1
    test(db)